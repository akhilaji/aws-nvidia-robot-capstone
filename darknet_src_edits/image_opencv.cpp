...
// Draw Detection
// ====================================================================
extern "C" void draw_detections_cv_v3(int global_frame_counter, FILE *results_output, mat_cv* mat, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    try {
                ...
                if (dets[i].prob[j] > thresh && show) {
                    if (class_id < 0) {
                        ...
                        if(results_output) {
                            ...
                            fprintf(results_output, "%d, %d, %0.2f, %f, %f, %f, %f\n",
                                global_frame_counter, class_id, dets[i].prob[j], b.x, b.y, (b.x + b.w), (b.y + b.h));
                        }
                        else {
                            fprintf(stderr, "Unable to open yolov3_obj_detection_results.txt at frame %d", global_frame_counter);
                        }
